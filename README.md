...
default or a custom `softmax_scale`, and uses direct math SDPA for repeatable
outputs and Q/K/V gradients. This deterministic path is global-only
(`window_size=(-1, -1)`) and does not support diagnostics, dropout, softcap, or
ALiBi; deterministic finite-window calls remain unsupported.
...
may additionally use fp32 ALiBi slopes shaped `[12]` or `[16, 12]`, with either
the default or a custom `softmax_scale`; this combination remains forward-only
and requires `deterministic=False`. Diagnostic calls use the generic packed
runtime, ALiBi-only calls retain their existing generic dispatch, and ordinary
calls retain the generated specialization. The same three entry points accept
`0 < dropout_p < 1` through PyTorch SDPA, with either scale. Causal mode,
ALiBi diagnostics on other profiles, deterministic diagnostics, dropout
diagnostics, and combinations with local windows or softcap remain outside this
BERT-base subset. For zero-dropout training without diagnostics, the same entry
points accept `deterministic=True` with either scale and call PyTorch's
...
Triton path; ALiBi backward is not implemented. The exact BERT-base profile
...
forward-only fp32 CUDA ALiBi slopes shaped `[12]` or `[16, 12]`. Forward-only
calls additionally accept `return_attn_probs=True`, with or without those
slopes, and return `(out, softmax_lse, S_dmask)`, with fp32 LSE shaped
`[12, total_q]` and an empty bf16 `S_dmask`, at the default or a custom scale.
ALiBi-only and diagnostic calls use their existing generic packed dispatch,
while ordinary `alibi_slopes=None` calls retain the existing generic inference
dispatch for both ragged and full-length inputs. Slope-free, global,
zero-dropout backward is additionally supported when
...
paging, dropout, windows, softcap, ALiBi backward, deterministic diagnostics,
and deterministic ragged backward remain explicitly unsupported.
...
fp32 ALiBi slopes shaped `[32]` or `[4, 32]`. Calls additionally accept
`return_attn_probs=True`, with or without those slopes, and return
`(out, softmax_lse, S_dmask)`, with fp32 LSE shaped `[32, total_q]` and an empty
bf16 `S_dmask`, at the default or a custom scale. Diagnostics support both
ragged self- and cross-attention and are inherited by the KV-packed adapter.
Ordinary slope-free, ALiBi-only, and slope-free diagnostic calls retain their
existing generic packed dispatch, and the registered dense specialization at
this shape retains generated dispatch. Gradients, dropout, windows, softcap,
deterministic mode, and paging remain unsupported, and
`is_varlen_shape_supported` remains false.
...
query and key cumulative offsets describing a mix of empty and nonempty query
or key slots, with at least one nonempty query and all lengths at most 512.
Causal self-attention also accepts identical query and key offsets containing
both empty and nonempty slots. Full-length inputs are viewed as one dense batch;
ragged causal inputs use one bounded PyTorch SDPA call per nonempty query
sequence with keys. A nonempty query slot with empty K/V instead returns exact
zero rows through a differentiable branch, with zero Q/K/V gradients matching
FA2. Both paths support the default or a custom `softmax_scale`. The QKV-packed
adapter inherits self-attention support; the KV-packed adapter accepts the same
independent query/key cases, including either zero-length query slots paired
with nonempty keys or nonempty query slots paired with empty K/V. Full-length
causal training additionally accepts the diagnostic return
...
to use the generated varlen kernel, including ragged and full-length calls. The
BERT-base varlen profile above likewise keeps its generic packed inference
dispatch for both layouts.
Ragged noncausal batches, batches with no nonempty queries, deterministic mode
outside the exact full-length BERT-base and batch-8 profiles described above,
paged caches, ALiBi, ragged diagnostic returns, and positive dropout remain
explicitly unsupported for varlen backward.
...
Three strict speculative-decoding slices use the generic packed Triton runtime
...
including the exact four- and eight-token profiles above; the exact causal
single-token 1K and 16K, four- and eight-token 1K, and either-causal 4K profiles
additionally accept their documented scalar prefixes. The exact causal
single-token 1K and 16K profiles and the 4K profile with either causal flag
accept their documented tensor-selected prefixes or left-padded spans.
...
16K, four- and eight-token 1K, and either-causal 4K read-only profiles,
non-final appends
...
BERT-base, the two exact full-length batch-8 profiles (including finite
symmetric-window backward on the noncausal profile and exact `(127, 0)`
causal-window backward on the causal profile), and the causal ragged query/key
contract described above, including empty-key requests through the unpacked and
KV-packed APIs; KV-cache calls remain forward-only. These unsupported
...
- KV-cache mutation beyond the dense paired one-token final-slot append, the
  ...
  device lengths of 1023 above;
  the exact four- and eight-token profiles are read-only;
  ...
  single-token 1K and 16K, four- and eight-token 1K, and either-causal 4K
  profiles above,
