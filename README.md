...
Every unregistered noncausal fp16/bf16 dense shape inside the generic Triton
...
with the input dtype. Diagnostic calls use the generic packed runtime;
`return_attn_probs=False` retains ordinary output-only dispatch. Generic causal
shapes and checked-in profiles other than the explicit diagnostic profiles
below remain unsupported for diagnostic returns.
...
specialization. Its causal mode, ALiBi, and other registered dense profiles
...
above, causal generic or non-diagnostic registered dense diagnostic returns,
...
- `return_attn_probs=True` for causal or out-of-envelope unregistered dense
