...
`head_dim <= 256`, the packed layouts and padded query indices fit signed
32-bit offsets, and the flattened query launch fits CUDA's X-grid limit. The
two catalogue entries remain specialization-accelerated; support queries report
those checked-in entries rather than generic fallback coverage.
...
core paged shapes raise `UnsupportedShapeError`. **To accelerate an unlisted shape, please
...
| [`b1_sq2048_sk2048_hq32_hkv32_d128_bf16_causal`](helion_attention/kernels/b1_sq2048_sk2048_hq32_hkv32_d128_bf16_causal.py) | unknown | incumbent (origin: autotuned) | unknown | 0.387584 ms | `helion.Config(atomic_indexing=[], block_sizes=[128, 64], indexing=['pointer', 'tensor_descriptor', 'pointer', 'tensor_descriptor'], l2_groupings=[32], load_eviction_policies=['first', 'last', 'first'], loop_orders=[[1, 0]], num_sm_multiplier=1, num_stages=3, num_warps=8, pid_type='persistent_interleaved', range_flattens=[None, None, None], range_multi_buffers=[None, None, True], range_num_stages=[0, 3, 1], range_unroll_factors=[0, 2, 3], range_warp_specializes=[])` | Helion 1.4.0, 8.8 min; candidate 0.428128 ms, incumbent 0.387584 ms<br>`helion.Config(atomic_indexing=[], block_sizes=[128, 64], indexing=['tensor_descriptor', 'tensor_descriptor', 'pointer', 'pointer'], l2_groupings=[64], load_eviction_policies=['first', 'first', ''], loop_orders=[[1, 0]], num_sm_multiplier=1, num_stages=5, num_warps=8, pid_type='persistent_interleaved', range_flattens=[None, None, True], range_multi_buffers=[None, False, True], range_num_stages=[0, 4, 1], range_unroll_factors=[0, 1, 4], range_warp_specializes=[])` |
...
| batch=1 seqlen_q=2048 seqlen_k=2048 nheads=32 head_dim=128 dtype=bf16 causal=True | 387 | 357 | 228 | 224 | 194 | 89 | **1.09x slower** |
...
Geomean speedup over FlashAttention 3 across all 34 comparable kernel workloads: **1.15x**.

Helion is the fastest measured implementation on 12 of 35 workloads; FA2, FA3, or a PyTorch SDPA backend is faster on the remainder.
<!-- BENCHMARKS:END -->
...
`(batch=8, seqlen=512, nheads=16, head_dim=64)` shape. Generic fallbacks, other
dense specializations, and packed varlen paths are forward-only and reject
grad-enabled calls at the call site. These unsupported FlashAttention features
...
helion_attention/          runtime package: torch + triton only
  ...
  _paged_attention.py      generic packed/paged Triton runtime and core fallbacks
