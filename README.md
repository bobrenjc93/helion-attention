...
`[0, 512, ..., 4096]`). That full-length case views the packed tensors as one
dense batch and uses PyTorch SDPA autograd for the forward and Q/K/V gradients.
The noncausal profile also supports ragged self-attention when all eight
sequences are nonempty and at most 512 tokens, and `cu_seqlens_q` and
`cu_seqlens_k` have identical values. It validates those offsets on the host
and runs eight bounded SDPA subproblems. Both the default and a custom
`softmax_scale` are supported. Full-length training supports CUDA graph
capture; ragged training rejects capture explicitly because its offsets are
dynamic host metadata. The varlen QKV/KV-packed adapters inherit the same
support. Calls that do not require gradients continue to use the generated
varlen kernel, including ragged and full-length calls. Ragged causal and
cross-attention offsets, deterministic, paged, ALiBi, diagnostic-return, and
positive-dropout varlen backward calls remain explicitly unsupported.
...
calls remain forward-only except for the two exact full-length profiles and
the bounded ragged noncausal self-attention profile described above; KV-cache
...
- backward for dense or varlen ALiBi, ragged causal/cross-attention varlen
  batches, and KV-cache calls
- `deterministic=True` when using a dense or varlen SDPA autograd fallback, and
  CUDA graph capture of the ragged varlen fallback
