...
GENERIC_DIAGNOSTIC_SPECS = [
    AttnShape(2, 23, 23, 4, 4, 32, torch.bfloat16, False),
    AttnShape(2, 23, 23, 4, 4, 32, torch.float16, False),
    AttnShape(2, 19, 19, 8, 2, 64, torch.bfloat16, False),
    AttnShape(2, 19, 19, 8, 2, 64, torch.float16, False),
    AttnShape(1, 29, 17, 4, 4, 128, torch.bfloat16, False),
    AttnShape(1, 29, 17, 4, 4, 128, torch.float16, False),
]
...
@pytest.mark.parametrize(
    "spec",
    GENERIC_DIAGNOSTIC_SPECS,
    ids=[spec.key for spec in GENERIC_DIAGNOSTIC_SPECS],
)
@pytest.mark.parametrize(
    "softmax_scale", [None, 0.37], ids=["default-scale", "custom-scale"]
)
def test_generic_noncausal_return_attn_probs_matches_fa2(
    spec: AttnShape,
    softmax_scale: float | None,
) -> None:
    flash_attn = pytest.importorskip("flash_attn")
...
@pytest.mark.parametrize(
    ("entry_point", "spec"),
    [
        pytest.param("qkvpacked", GENERIC_DIAGNOSTIC_SPECS[0], id="qkv-bf16"),
        pytest.param("qkvpacked", GENERIC_DIAGNOSTIC_SPECS[1], id="qkv-fp16"),
        pytest.param("kvpacked", GENERIC_DIAGNOSTIC_SPECS[2], id="gqa-bf16"),
        pytest.param("kvpacked", GENERIC_DIAGNOSTIC_SPECS[3], id="gqa-fp16"),
        pytest.param("kvpacked", GENERIC_DIAGNOSTIC_SPECS[4], id="cross-bf16"),
        pytest.param("kvpacked", GENERIC_DIAGNOSTIC_SPECS[5], id="cross-fp16"),
    ],
)
@pytest.mark.parametrize(
    "softmax_scale", [None, 0.37], ids=["default-scale", "custom-scale"]
)
def test_generic_noncausal_packed_adapters_inherit_diagnostic_return(
    entry_point: str,
    spec: AttnShape,
    softmax_scale: float | None,
) -> None:
...
@pytest.mark.parametrize(
    ("entry_point", "spec"),
    [
        pytest.param("dense", GENERIC_DIAGNOSTIC_SPECS[0], id="dense"),
        pytest.param("qkvpacked", GENERIC_DIAGNOSTIC_SPECS[0], id="qkv-packed"),
        pytest.param("kvpacked", GENERIC_DIAGNOSTIC_SPECS[4], id="kv-packed"),
    ],
)
def test_generic_noncausal_return_attn_probs_false_retains_output_dispatch(
    entry_point: str,
    spec: AttnShape,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_paged_kvcache_alibi_matches_fa2_and_fp32_for_ragged_permuted_pages(
    ...
    q, k_cache, v_cache, cache_seqlens, block_table, logical_caches = (
        make_paged_kvcache_inputs(seed=20260808)
    )
    original_k = k_cache.clone()
    original_v = v_cache.clone()
