...
LLAMA3_2K_VARLEN_INFERENCE = AttnShape(
    batch=4,
    seqlen_q=2048,
    seqlen_k=2048,
    nheads_q=32,
...
MIXED_EMPTY_CROSS_KEY_LENGTHS = [3, 0, 97, 191, 0, 319, 443, 0]
...
def reference_softcap_packed_diagnostics(
    ...
    *,
    ...
    for seqlen_q, seqlen_k in zip(lengths_q, lengths_k):
        ...
        # FA2 reports +inf, rather than the mathematical -inf, for query rows
        # that have no visible keys. Match that diagnostic convention while
        # retaining a direct fp32 calculation for every nonempty row.
...
@pytest.mark.parametrize(
    "empty_slots",
    ["none", "query", "key", "all-key"],
    ids=["nonempty", "empty-query", "empty-key", "all-empty-key"],
)
def test_ragged_causal_varlen_cross_attention_backward_matches_fp32_and_fa2(
    ...
    requested_lengths_q = (
        MIXED_EMPTY_CROSS_QUERY_LENGTHS if empty_slots == "query" else None
    )
    requested_lengths_k = (
        MIXED_EMPTY_CROSS_KEY_LENGTHS
        if empty_slots == "key"
        else [0] * spec.batch
        if empty_slots == "all-key"
        else None
    )
    q, k, v, cu_q, cu_k, lengths_q, lengths_k = make_ragged_cross_packed(
        spec,
        seed=20260809,
        lengths_q=requested_lengths_q,
        lengths_k=requested_lengths_k,
    )
    ...
    if empty_slots == "query":
        ...
        for query_length, key_length in zip(lengths_q, lengths_k):
            ...
            k_start += key_length
    elif empty_slots in {"key", "all-key"}:
        q_grad = got_grads[0]
        q_start = 0
        for query_length, key_length in zip(lengths_q, lengths_k):
            query_slice = slice(q_start, q_start + query_length)
            if key_length == 0:
                assert torch.count_nonzero(got[query_slice]).item() == 0
                assert torch.count_nonzero(q_grad[query_slice]).item() == 0
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("noncausal-empty-key", "causal.*profile"),
        ...
        ("deterministic-empty-key-cross", "deterministic=True"),
    ],
)
def test_ragged_varlen_backward_rejects_out_of_scope_calls(
    ...
    elif case in {"noncausal-empty-key", "deterministic-empty-key-cross"}:
        q, k, v, cu_q, cu_k, *_ = make_ragged_cross_packed(
            spec, lengths_k=MIXED_EMPTY_CROSS_KEY_LENGTHS
        )
    else:
...
@pytest.mark.parametrize(
    "attention",
    ["self", "self-empty", "cross-empty-query", "cross-empty-key"],
    ids=["self", "empty-self", "empty-query-cross", "empty-key-cross"],
)
def test_ragged_causal_varlen_backward_rejects_incompatible_options(
    ...
    spec = VARLEN_ALIBI_CAUSAL
    if attention in {"cross-empty-query", "cross-empty-key"}:
        q, k, v, cu_q, cu_k, *_ = make_ragged_cross_packed(
            spec,
            lengths_q=(
...
def test_llama3_2k_varlen_fallback_does_not_change_registration() -> None:
    spec = LLAMA3_2K_VARLEN_INFERENCE
    ...
    assert f"varlen_{spec.key}" not in VARLEN_IDS
...
def test_llama3_2k_varlen_unpacked_matches_fp32_and_fa2(
    attention: str, softmax_scale: float | None
) -> None:
    flash_attn = pytest.importorskip("flash_attn")
    spec = LLAMA3_2K_VARLEN_INFERENCE
    if attention == "self":
        q, k, v, cu_q, lengths_q = make_ragged_self_packed(
            spec, lengths=[2048, 257, 31, 0], seed=20260811
        )
        cu_k = cu_q
        lengths_k = lengths_q
    else:
        q, k, v, cu_q, cu_k, lengths_q, lengths_k = make_ragged_cross_packed(
            spec,
            lengths_q=[2048, 257, 31, 7],
            lengths_k=[31, 2048, 17, 511],
            seed=20260811,
        )
    scale = (
        1.0 / math.sqrt(spec.head_dim)
    ...
    assert max(lengths_q) == max(lengths_k) == 2048
    assert got.shape == q.shape
...
def test_llama3_2k_varlen_diagnostics_match_fa2(
    entry_point: str,
    attention: str,
    softmax_scale: float | None,
) -> None:
...
def test_llama3_2k_varlen_alibi_diagnostics_match_fa2(
    attention: str,
    batched_slopes: bool,
    softmax_scale: float | None,
) -> None:
...
def test_llama3_2k_varlen_alibi_matches_fp32_and_fa2(
    attention: str,
    batched_slopes: bool,
    softmax_scale: float | None,
) -> None:
...
def test_llama3_2k_varlen_kvpacked_inherits_support(
    attention: str, softmax_scale: float | None
) -> None:
    flash_attn = pytest.importorskip("flash_attn")
    spec = LLAMA3_2K_VARLEN_INFERENCE
    if attention == "self":
        q, k, v, cu_q, _ = make_ragged_self_packed(
            spec, lengths=[2048, 257, 31, 0], seed=271828
        )
        cu_k = cu_q
    else:
        q, k, v, cu_q, cu_k, *_ = make_ragged_cross_packed(
    ...
    torch.testing.assert_close(got, unpacked)
    torch.testing.assert_close(
        got.float(), expected_fa2.float(), atol=5e-2, rtol=2e-2
    )
...
def test_llama3_2k_varlen_kvpacked_inherits_alibi_support(
    batched_slopes: bool, softmax_scale: float | None
) -> None:
    spec = LLAMA3_2K_VARLEN_INFERENCE
    q, k, v, cu_q, cu_k, *_ = make_ragged_cross_packed(
        spec,
        lengths_q=[128, 31, 7, 1],
        lengths_k=[17, 256, 9, 63],
        seed=271828,
    )
    head_slopes = torch.linspace(0.01, 0.2, spec.nheads_q, device=q.device)
    slopes = (
...
def test_llama3_2k_varlen_kvpacked_inherits_alibi_diagnostics(
    batched_slopes: bool, softmax_scale: float | None
) -> None:
    flash_attn = pytest.importorskip("flash_attn")
    spec = LLAMA3_2K_VARLEN_INFERENCE
...
def test_llama3_2k_varlen_routes_only_through_generic_inference(
    entry_point: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    spec = LLAMA3_2K_VARLEN_INFERENCE
    q, k, v, cu_seqlens, _ = make_ragged_self_packed(
        spec, lengths=[1, 1, 1, 1]
    )
    sentinel = torch.empty_like(q)
    calls: list[tuple[object, ...]] = []
    ...
    def reject_generated(*args: object, **kwargs: object) -> object:
        raise AssertionError("2K varlen inference reached generated dispatch")
...
@pytest.mark.parametrize(
    "spec",
    [
        pytest.param(LLAMA3_VARLEN_INFERENCE, id="256-token"),
        pytest.param(LLAMA3_2K_VARLEN_INFERENCE, id="2k-token"),
    ],
)
def test_llama3_varlen_alibi_profiles_share_existing_generic_dispatch(
    spec: AttnShape, monkeypatch: pytest.MonkeyPatch
) -> None:
    q, k, v, cu_seqlens, _ = make_ragged_self_packed(
        spec, lengths=[1, 1, 1, 1]
    ...
    sentinel = torch.empty_like(q)
    calls: list[tuple[float, AttnShape, torch.Tensor]] = []
    ...
    def generic_alibi(
        ...
        calls.append((scale_arg, spec_arg, slopes_arg))
        return sentinel

    def reject_dispatch(*args: object, **kwargs: object) -> object:
        raise AssertionError("Llama-3 ALiBi call changed generic dispatch")

    monkeypatch.setattr(
        helion_attention, "_generic_varlen_alibi_forward", generic_alibi
    )
    monkeypatch.setattr(
        helion_attention, "_generic_varlen_forward", reject_dispatch
    )
    ...
    assert got is sentinel
    assert calls == [(0.37, spec, slopes)]


@requires_cuda
@pytest.mark.parametrize(
    ("with_alibi", "gradient_source", "message"),
    [
        pytest.param(False, "q", "varlen backward", id="plain-q"),
        pytest.param(True, "q", "ALiBi backward", id="alibi-q"),
        pytest.param(True, "slopes", "ALiBi backward", id="alibi-slopes"),
    ],
)
def test_llama3_2k_varlen_rejects_gradients_before_dispatch(
    with_alibi: bool,
    gradient_source: str,
    message: str,
    monkeypatch: pytest.MonkeyPatch,
    ...
    slopes = torch.ones(spec.nheads_q, device=q.device) if with_alibi else None
    ...
    def reject_dispatch(*args: object, **kwargs: object) -> torch.Tensor:
        raise AssertionError("2K varlen gradient reached generic dispatch")
...
@pytest.mark.parametrize("with_alibi", [False, True], ids=["plain", "alibi"])
def test_llama3_2k_varlen_rejects_optional_features_before_dispatch(
    option: str,
    value: object,
    message: str,
    with_alibi: bool,
    ...
    def reject_dispatch(*args: object, **kwargs: object) -> torch.Tensor:
        raise AssertionError("unsupported 2K varlen option reached dispatch")
    ...
    kwargs: dict[str, object] = {option: value}
    if with_alibi:
        kwargs["alibi_slopes"] = torch.ones(spec.nheads_q, device=q.device)
    with pytest.raises(NotImplementedError, match=message):
        helion_attention.flash_attn_varlen_func(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "slope-gradient", "ALiBi backward", id="slope-gradient"
        ),
        pytest.param("dropout", "dropout", id="dropout"),
        pytest.param("window", "sliding-window", id="window"),
        pytest.param("softcap", "softcap", id="softcap"),
...
def test_llama3_2k_varlen_alibi_diagnostics_reject_incompatible_features(
    case: str,
    message: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def reject_dispatch(*args: object, **dispatch_kwargs: object) -> object:
        raise AssertionError("incompatible 2K diagnostic reached dispatch")
...
def test_llama3_2k_varlen_rejects_paging_before_dispatch(
    with_alibi: bool,
    exception: type[Exception],
    message: str,
    monkeypatch: pytest.MonkeyPatch,
    ...
    k = torch.zeros(1, 16, 8, 128, device=q.device, dtype=spec.dtype)
    v = torch.zeros_like(k)
    cu_seqlens = torch.arange(5, device=q.device, dtype=torch.int32)
    block_table = torch.zeros(4, 128, device=q.device, dtype=torch.int32)

    def reject_dispatch(*args: object, **kwargs: object) -> object:
        raise AssertionError("2K varlen paging reached attention dispatch")
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            AttnShape(4, 2047, 2047, 32, 8, 128, torch.bfloat16, True),
            id="other-maxima",
        ),
        pytest.param(
            AttnShape(4, 2048, 2047, 32, 8, 128, torch.bfloat16, True),
            id="unequal-maxima",
        ),
        pytest.param(
            AttnShape(4, 2048, 2048, 32, 8, 128, torch.bfloat16, False),
            id="noncausal",
        ),
        pytest.param(
            AttnShape(4, 2048, 2048, 32, 8, 128, torch.float16, True),
            id="fp16",
        ),
    ],
)
@pytest.mark.parametrize(
    "feature", ["plain", "alibi", "diagnostics", "combined"]
)
def test_llama3_2k_varlen_neighboring_profiles_remain_unsupported(
    spec: AttnShape,
    feature: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    q = torch.zeros(
        spec.batch, spec.nheads_q, spec.head_dim, device="cuda", dtype=spec.dtype
    )
    k = torch.zeros(
        spec.batch, spec.nheads_kv, spec.head_dim, device=q.device, dtype=spec.dtype
    )
    v = torch.zeros_like(k)
    cu_seqlens = torch.arange(
        spec.batch + 1, device=q.device, dtype=torch.int32
    ...
    def reject_generic(*args: object, **kwargs: object) -> torch.Tensor:
        raise AssertionError("neighboring profile reached 2K varlen dispatch")
    ...
    monkeypatch.setattr(
        helion_attention, "_generic_varlen_diagnostic_forward", reject_generic
    )
    slopes = (
        torch.ones(spec.nheads_q, device=q.device)
        if feature in ("alibi", "combined")
        else None
    )
    exception = (
        NotImplementedError
        if feature != "plain"
        else helion_attention.UnsupportedShapeError
    )
    message = "implemented only" if feature != "plain" else "checked in"
    with pytest.raises(exception, match=message):
        helion_attention.flash_attn_varlen_func(
            q,
            k,
            ...
            causal=spec.causal,
            alibi_slopes=slopes,
            return_attn_probs=feature in ("diagnostics", "combined"),
            shape=spec,
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(LLAMA3_2K_VARLEN_INFERENCE, id="llama3-2k-gqa"),
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            VARLEN_SOFTCAP_VALUE,
            None,
            id="cap-50-default-scale",
        ),
        pytest.param(
            VARLEN_SOFTCAP_VALUE,
            0.37,
            id="cap-50-custom-scale",
        ),
        pytest.param(
            VARLEN_NON_50_SOFTCAP,
            None,
...
def test_causal_varlen_softcap_diagnostics_match_fa2_and_fp32(
    attention: str,
    softcap: float,
    softmax_scale: float | None,
) -> None:
    ...
    assert softmax_lse.shape == (spec.nheads_q, q.shape[0])
    ...
    assert expected_out.dtype == out.dtype
    assert expected_lse.shape == softmax_lse.shape
    assert expected_lse.dtype == softmax_lse.dtype
    assert expected_s_dmask.shape == s_dmask.shape
    assert expected_s_dmask.dtype == s_dmask.dtype
    assert expected_s_dmask.device == s_dmask.device
    torch.testing.assert_close(
        out.float(), expected_out.float(), atol=1e-2, rtol=1e-2
    )
    finite_lse = torch.isfinite(expected_lse)
...
@pytest.mark.parametrize(
    "features",
    ["softcap", "diagnostics", "combined-cap-50", "combined-non-50"],
)
def test_varlen_softcap_and_diagnostic_dispatch_stays_separate(
    ...
    kwargs: dict[str, object] = {}
    if features == "softcap":
        kwargs["softcap"] = VARLEN_NON_50_SOFTCAP
    elif features == "combined-cap-50":
        kwargs["softcap"] = VARLEN_SOFTCAP_VALUE
    elif features == "combined-non-50":
        kwargs["softcap"] = VARLEN_NON_50_SOFTCAP
    if features != "softcap":
        kwargs["return_attn_probs"] = True
    with torch.no_grad():
    ...
    assert len(args) == 7
    expected_kwargs = (
        {} if features == "diagnostics" else {"softcap": kwargs["softcap"]}
    )
    assert dispatch_kwargs == expected_kwargs
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            LLAMA3_2K_VARLEN_INFERENCE,
            True,
            id="llama3-2k-alibi-diagnostics",
        ),
    ],
)
...
def test_core_varlen_paged_diagnostics_match_fa2_for_ragged_permuted_pages(
    causal: bool, softmax_scale: float | None, page_size: int
) -> None:
    flash_attn = pytest.importorskip("flash_attn")
    q, k, v, cu_q, cu_k, block_table, _, request_kv = make_paged_decode(
    ...
    fa2_k, fa2_v, fa2_block_table = k, v, block_table
    if page_size == 16:
        # FA2 requires physical cache pages divisible by 256. Re-page the same
        # seeded logical caches for its reference call while exercising page 16
        # through helion-attention above.
        _, fa2_k, fa2_v, _, _, fa2_block_table, _, _ = make_paged_decode(
...
@pytest.mark.parametrize(
    ("page_size", "softcap"),
    [
        pytest.param(16, 0.0, id="page-16"),
...
def test_core_varlen_paged_diagnostics_request_lse_from_generic_runtime(
    monkeypatch: pytest.MonkeyPatch, page_size: int, softcap: float
) -> None:
    import helion_attention._paged_attention as generic_attention
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "page-128",
            "page-size-16, page-size-256, or page-size-512",
            id="other-page",
        ),
        ...
        pytest.param("page-16-alibi", "ALiBi", id="page-16-alibi"),
...
def test_core_varlen_paged_diagnostics_reject_out_of_scope_calls_before_dispatch(
    case: str, message: str, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    scoped_case = case
    if case.startswith("page-16-"):
        q, k, v, cu_q, cu_k, block_table, *_ = make_paged_decode(
            page_size=16
        )
        kwargs["block_table"] = block_table
        scoped_case = case.removeprefix("page-16-")
    elif case.startswith("page-512-"):
        q, k, v, cu_q, cu_k, block_table, *_ = make_paged_decode(
            page_size=512
        )
        kwargs["block_table"] = block_table
