...
@requires_cuda
def test_full_noncausal_varlen_dropout_matches_direct_sdpa_rng_and_gradients() -> None:
    ...
    expected = torch.nn.functional.scaled_dot_product_attention(
        ...
        dropout_p=dropout_p,
        scale=scale,
    ).transpose(1, 2).contiguous().reshape(q.shape)
...
def test_full_noncausal_varlen_packed_dropout_matches_direct_sdpa(
    name: str,
) -> None:
    spec = VARLEN_ALIBI_NONCAUSAL
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("causal", "only for eight full-length sequences"),
...
def test_varlen_dropout_rejects_incompatible_calls_before_dispatch(
    ...
    kwargs: dict[str, object] = {
        "dropout_p": 0.25,
        "shape": VARLEN_ALIBI_NONCAUSAL,
    }
    if case == "other-shape":
        kwargs["shape"] = (1, 1, 1, 1)
        cu_seqlens = torch.zeros(2, dtype=torch.int32)
    elif case == "causal":
        kwargs.update(causal=True, shape=VARLEN_ALIBI_CAUSAL)
    ...
    with pytest.raises(NotImplementedError, match=message):
        helion_attention.flash_attn_varlen_func(
            ...
            cu_seqlens,
            cu_seqlens,
            512 if case != "other-shape" else 1,
            512 if case != "other-shape" else 1,
