...
def test_causal_ragged_varlen_packed_entry_points_reject_gradients(
    name: str,
) -> None:
...
def test_ragged_noncausal_varlen_backward_matches_fp32_and_fa2(
...
def test_ragged_noncausal_varlen_packed_adapters_match_fp32_and_fa2(
...
def test_ragged_noncausal_no_grad_retains_generated_dispatch(
...
def test_ragged_varlen_backward_rejects_out_of_scope_calls(
    ...
    spec = (
        VARLEN_ALIBI_CAUSAL
        if case == "causal"
        else VARLEN_ALIBI_NONCAUSAL
    )
    if case == "cross-attention":
        q, k, v, cu_q, cu_k, *_ = make_packed(spec, variant=0)
    elif case == "empty":
        q, k, v, _, _ = make_ragged_self_packed(spec)
        lengths = [512, 401, 300, 255, 128, 63, 18, 0]
        cu_q = _cumulative(lengths, q.device)
        cu_k = cu_q
    else:
...
@requires_cuda
def test_ragged_noncausal_backward_rejects_cuda_graph_capture(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    spec = VARLEN_ALIBI_NONCAUSAL
    q, k, v, cu_seqlens, _ = make_ragged_self_packed(spec)
