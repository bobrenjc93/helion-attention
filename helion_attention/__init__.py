...
calls without a generated backward, both full-length varlen profiles, and
ragged noncausal varlen self-attention use PyTorch SDPA autograd. The explicit
...
def _ragged_varlen_self_attention_lengths(
    ...
    if torch.cuda.is_current_stream_capturing():
        raise NotImplementedError(
            "ragged varlen backward is not supported during CUDA graph capture"
        )
...
def flash_attn_varlen_func(
    ...
    *,
    ...
    key sequences have length 512. The noncausal profile additionally supports
    ...
    Ragged causal/cross-attention, graph-captured ragged, deterministic, paged,
    ALiBi, and diagnostic varlen backward remain unsupported. Calls that do not
    need backward retain the generated packed kernel, including ragged and
    full-length calls.
    """
    _reject_unsupported(
        dropout_p,
    ...
    if needs_backward:
        ...
        if (
            ...
            raise NotImplementedError(
                "ragged varlen backward is implemented only for the noncausal "
                "bf16 (8, 512, 512, 16, 16, 64) self-attention profile; causal "
                "ragged batches remain forward-only"
            )
        if softmax_scale is None:
            softmax_scale = 1.0 / math.sqrt(spec.head_dim)
        scale = float(softmax_scale)
