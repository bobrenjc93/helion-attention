...
profiles with optional ALiBi diagnostics, ALiBi on both shipped varlen profiles
...
def _is_llama_2k_left_window(window_size: tuple[object, ...]) -> bool:
    """Whether ``window_size`` is in the narrow dense Llama 2K range."""
    ...
    return (
        ...
        and 0 <= left < _LLAMA_2K_LEFT_WINDOW_UPPER_BOUND
        and right == 0
    )
...
def _check_core_paged_varlen_spec(spec: AttnShape, page_size: int) -> None:
    ...
    supported_page_size = (
        ...
        or (
            ...
            and page_size in _CORE_PAGED_VARLEN_DECODE_PAGE_SIZES
        )
    )
    if (
...
def _check_core_paged_kvcache_spec(spec: AttnShape, page_size: int) -> None:
    ...
    supported_page_size = (
        ...
        or (
            requested == _CORE_PAGED_KVCACHE_SHAPE
            and page_size in _PAGED_KVCACHE_DECODE_PAGE_SIZES
        )
    )
    if (
...
def _check_core_paged_varlen_softcap_spec(
    ...
    """Restrict core paged-varlen softcap to validated large-page decode."""
    requested = (
        spec.batch,
        spec.seqlen_q,
        spec.seqlen_k,
    ...
    if requested != _CORE_PAGED_KVCACHE_SHAPE or page_size not in (256, 512):
        raise NotImplementedError(
            ...
            "no-backward bf16 page-size-256 or page-size-512 batch=4 "
...
def _generic_paged_varlen_forward(
    ...
    *,
    ...
    """Run validated paged decode through the generic Triton runtime."""
    # Page 16 output-only calls retain their checked-in specialization; only
    # their diagnostic return joins page 256/512 here. Other page sizes and
    # paged profiles still fail before reaching the generic vLLM runtime.
    ...
    packed_out, softmax_lse = result
    return packed_out, softmax_lse, q.new_empty((0,))
    ...
    *,
    ...
    Args:
        ...
        window_size: ``(-1, -1)`` selects global attention. ``(left, 0)`` is
            additionally supported for every Python integer
    ...
    kernel. The Llama GQA local-window calls described above also use that
    ...
    if has_left_window:
        if spec.key != _LLAMA_2K_LEFT_WINDOW_KEY:
            raise NotImplementedError(
                ...
                "(1, 2048, 2048, 32, 8, 128) with window_size=(left, 0) "
                "where 0 <= left < 2048; "
                f"got {spec.describe()}"
            )
        if dropout != 0.0:
...
def flash_attn_varlen_func(
    ...
    *,
    ...
    paged runtime; page-size-16 output-only calls retain generated dispatch.
    Page-size-256 and page-size-512 decode also accept either forward-only fp32
    ...
    either causal flag, a default or custom ``softmax_scale``, optional ALiBi,
    or large-page softcap.
    ...
    ``(16, 512, 512, 12, 12, 64)`` supports arbitrary ragged lengths when no
    backward is required. Device-resident query and key offsets may vary
    ...
    ``softmax_scale`` is accepted. Calls additionally accept
    ``return_attn_probs=True`` and return fp32 LSE shaped ``[32, total_q]``
    plus an empty bf16 ``S_dmask``. Forward-only fp32 ALiBi slopes shaped
    ``[32]`` or ``[4, 32]`` may be supplied with or without that diagnostic
    return through the same generic packed runtime used by the 256-token
    profile. Ordinary slope-free, ALiBi-only, and slope-free diagnostic calls
    retain their existing dispatch. Dropout, windows, softcap, deterministic
    mode, paging, and backward remain unsupported for this deliberately
    unregistered profile. Its dense generated specialization and every
    registered varlen specialization retain their existing dispatch.
    ...
    if return_attn_probs:
        if block_table is not None:
            ...
            if requested != _CORE_PAGED_KVCACHE_SHAPE:
                raise NotImplementedError(
                    "return_attn_probs=True with block_table is implemented "
                    "only for no-backward bf16 page-size-16, page-size-256, "
                    "or page-size-512 batch=4 "
                    "seqlen_q=1 seqlen_k=1024 nheads=8 (GQA 8:2) "
                    "head_dim=128 decode"
                )
        else:
            requested = f"varlen_{spec.key}"
            if (
                ...
                raise NotImplementedError(
                    "return_attn_probs=True with ALiBi slopes is implemented "
                    f"only for {supported}"
                )
            if deterministic:
                raise NotImplementedError(
                    "return_attn_probs=True requires deterministic=False"
            ...
            if not _supports_varlen_diagnostic_return(spec):
                supported = ", ".join(sorted(_VARLEN_DIAGNOSTIC_KEYS))
                raise NotImplementedError(
    ...
    if block_table is not None:
        ...
        if (
            return_attn_probs
            and page_size not in _CORE_PAGED_VARLEN_DIAGNOSTIC_PAGE_SIZES
        ):
            raise NotImplementedError(
        ...
        if page_size != _CORE_PAGED_GENERATED_PAGE_SIZE or return_attn_probs:
            ...
            if return_attn_probs:
                generic_kwargs["return_attn_probs"] = True
    ...
    if needs_backward:
        ...
        if f"varlen_{spec.key}" not in _VARLEN_SDPA_BACKWARD_KEYS:
            ...
            raise NotImplementedError(
                ...
                f"{supported} under their validated full-length or "
                "causal-ragged contracts"
            )
        if deterministic and (
            f"varlen_{spec.key}" not in _VARLEN_DETERMINISTIC_BACKWARD_KEYS
            or not full_length
        ):
            supported = ", ".join(sorted(_VARLEN_DETERMINISTIC_BACKWARD_KEYS))
            raise NotImplementedError(
    ...
    *,
    ...
    if return_softmax_lse and requested != _CORE_PAGED_KVCACHE_SHAPE:
        raise NotImplementedError(
            ...
            "for the bf16 page-size-16, page-size-256, or page-size-512 "
            "batch=4 seqlen_q=1 seqlen_k=1024 "
    ...
    if alibi_slopes is not None:
        ...
        if not supports_alibi_page_size:
            raise NotImplementedError(
                ...
                "caches or page-size-256/page-size-512 caches for the decode "
                "profile"
            )
        if requested not in _CORE_PAGED_KVCACHE_SHAPES:
            raise NotImplementedError(
                ...
                "chunked-prefill and page-size-16, page-size-256, or "
