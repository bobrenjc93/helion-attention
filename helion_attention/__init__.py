...
ALiBi calls, ALiBi on one shipped causal varlen profile, and diagnostics on its
shipped noncausal counterpart use a generic Triton forward kernel. Positive
...
def flash_attn_varlen_func(
    ...
    *,
    ...
    The noncausal version of that profile also supports
    ``return_attn_probs=True`` when no backward is needed and all options other
    than ``softmax_scale`` retain their defaults. It returns ``(out,
    softmax_lse, S_dmask)`` with fp32 LSE shaped ``[nheads_q, total_q]`` and an
    empty bf16 ``S_dmask``, matching FlashAttention 2 when dropout is zero.
    """
    _reject_unsupported(
        dropout_p,
        window_size,
