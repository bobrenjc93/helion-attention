...
ALiBi calls, BERT-base and unregistered causal dense diagnostics, ALiBi on both
shipped varlen profiles, and diagnostics on the shipped causal varlen profile
use a generic Triton forward kernel. That runtime also provides
...
def _supports_diagnostic_return(spec: AttnShape) -> bool:
    ...
    # The diagnostic helper adapts the same packed runtime used by the ordinary
    # generic dense forward. Keep checked-in non-diagnostic profiles on their
    # exact gate, and do not broaden generic noncausal support.
...
def flash_attn_func(
    ...
    *,
    ...
    Args:
        ...
        return_attn_probs: for compatible unregistered causal dense shapes,
    ...
    if return_attn_probs:
        ...
        if not _supports_diagnostic_return(spec):
            raise NotImplementedError(
                ...
                "causal shapes supported by the generic dense fallback, the "
                f"shipped {_BERT_DIAGNOSTIC_KEY} BERT-base encoder, and the "
                "three shipped batch-1, single-token bf16 Llama GQA decode "
                "profiles"
            )
