...
Autotuning provenance:
    ...
    Autotuning wall time: 1302.098 s
    Measured time: 1.643280 ms
...
def _helion_causal_attention_bshd(q, k, v, out, qk_scale, _RDIM_SIZE_3: tl.constexpr):
    ...
    for offset_2 in tl.range(0, 4096, _BLOCK_SIZE_2):
        ...
        for offset_4 in tl.range(0, tl.cast(tile_end, tl.int32), _BLOCK_SIZE_4, disallow_acc_multi_buffer=True, flatten=True):
            ...
            v_blk = tl.load(v + (offset_0 * 16777216 + indices_4[:, None] * 4096 + offset_1 * 128 + (0 + indices_3)[None, :] * 1), mask_4[:, None], other=0)
...
def attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sm_scale: float, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_causal_attention_bshd, (4 * 32,), q, k, v, out, qk_scale, _RDIM_SIZE_3, num_warps=16, num_stages=3)
