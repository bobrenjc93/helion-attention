...
Autotuning provenance:
    ...
    Autotuning wall time: 1331.075 s
    Measured time: 1.391712 ms
...
def _helion_causal_attention_bshd(q, k, v, out, qk_scale, _NUM_SM: tl.constexpr, _RDIM_SIZE_3: tl.constexpr):
    ...
    v_desc = tl.make_tensor_descriptor(v, [1, 4096, 4, 128], [2097152, 512, 128, 1], [1, _BLOCK_SIZE_4, 1, 128])
    ...
    total_pids = 28 * 1
    for virtual_pid in tl.range(tl.program_id(0), total_pids, _NUM_SM * 128, disallow_acc_multi_buffer=True, flatten=True):
        # src[helion_kernels.py:233]: for b, h in hl.grid([batch, nheads_q]):
        num_pid_m = 28
        num_pid_n = 1
        ...
        for offset_2 in tl.range(0, 4096, _BLOCK_SIZE_2, flatten=True):
            ...
            out_desc.store([offset_0, offset_2, offset_1, 0 + 0], tl.cast(tl.reshape(v_20, [1, _BLOCK_SIZE_2, 1, _RDIM_SIZE_3]), tl.bfloat16))
...
def attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sm_scale: float, *, _launcher=_default_launcher):
    ...
    # src[helion_kernels.py:235]:     for tile_m in hl.tile(m_dim):
    # src[helion_kernels.py:233-270]: ...
    _launcher(_helion_causal_attention_bshd, (_NUM_SM * 128,), q, k, v, out, qk_scale, _NUM_SM, _RDIM_SIZE_3, num_warps=8, num_stages=3, maxnreg=256)
