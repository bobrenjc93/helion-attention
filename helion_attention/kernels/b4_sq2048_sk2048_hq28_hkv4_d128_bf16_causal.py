...
def _helion_causal_attention_bshd(q, k, v, out, qk_scale, _NUM_SM: tl.constexpr, _RDIM_SIZE_3: tl.constexpr):
    ...
    v_desc = tl.make_tensor_descriptor(v, [4, 2048, 4, 128], [1048576, 512, 128, 1], [1, _BLOCK_SIZE_4, 1, 128])
    # src[helion_kernels.py:270]: out[b, tile_m, h, :] = result.to(out.dtype)
    out_desc = tl.make_tensor_descriptor(out, [4, 2048, 28, 128], [7340032, 3584, 128, 1], [1, _BLOCK_SIZE_2, 1, 128])
    ...
    total_pids = 4 * 28
    for virtual_pid in tl.range(tl.program_id(0), total_pids, _NUM_SM, flatten=False):
        # src[helion_kernels.py:233]: for b, h in hl.grid([batch, nheads_q]):
        num_pid_m = 4
        num_pid_n = 28
        ...
        for offset_2 in tl.range(0, 2048, _BLOCK_SIZE_2):
            ...
            for offset_4 in tl.range(0, tl.cast(tile_end, tl.int32), _BLOCK_SIZE_4, disallow_acc_multi_buffer=True, flatten=False):
