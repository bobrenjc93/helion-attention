...
Autotuning provenance:
    ...
    Autotuning wall time: 320.532 s
    Measured time: 0.014496 ms
...
def _helion_causal_attention_bshd(q, k, v, out, softmax_lse, qk_scale, _RDIM_SIZE_3: tl.constexpr, STORE_LSE: tl.constexpr):
    ...
    for offset_2 in tl.static_range(0, 1):
        ...
        for offset_4 in tl.range(0, 1024, _BLOCK_SIZE_4, loop_unroll_factor=1, num_stages=3, disallow_acc_multi_buffer=False, flatten=False):
            ...
            v_blk = tl.load(v + (indices_4[:, None] * 1024 + symnode_0 * 128 + (0 + indices_3)[None, :] * 1), None, eviction_policy='evict_last')
...
def attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sm_scale: float, *, return_softmax_lse: bool = False, _launcher=_default_launcher):
    ...
    _launcher(_helion_causal_attention_bshd, (1 * 32,), q, k, v, out, softmax_lse, qk_scale, _RDIM_SIZE_3, STORE_LSE=return_softmax_lse, num_warps=4, num_stages=1)
