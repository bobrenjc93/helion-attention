...
Autotuning provenance:
    ...
    Autotuning wall time: 1265.970 s
    Measured time: 1.449872 ms
...
def _helion_causal_attention_bshd(q, k, v, out, qk_scale, _NUM_SM: tl.constexpr, _RDIM_SIZE_3: tl.constexpr):
    ...
    out_desc = tl.make_tensor_descriptor(out, [1, 4096, 32, 128], [16777216, 4096, 128, 1], [1, _BLOCK_SIZE_2, 1, 128])
    ...
    total_pids = 32 * 1
    for virtual_pid in tl.range(tl.program_id(0), total_pids, _NUM_SM, disallow_acc_multi_buffer=False):
        # src[helion_kernels.py:233]: for b, h in hl.grid([batch, nheads_q]):
        num_pid_m = 32
        num_pid_n = 1
...
def attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sm_scale: float, *, _launcher=_default_launcher):
    ...
    # src[helion_kernels.py:235]:     for tile_m in hl.tile(m_dim):
    # src[helion_kernels.py:233-270]: ...
    _launcher(_helion_causal_attention_bshd, (_NUM_SM,), q, k, v, out, qk_scale, _NUM_SM, _RDIM_SIZE_3, num_warps=8, num_stages=3, maxnreg=256)
