...
Regenerate with:
    python tools/generate.py --batch 8 --seqlen 512 --nheads 16 --head-dim 64 --dtype bf16
...
def _helion_attention_bshd(q, k, v, out, qk_scale, _NUM_SM: tl.constexpr, _RDIM_SIZE_3: tl.constexpr):
    ...
    for virtual_pid in tl.range(tl.program_id(0), total_pids, _NUM_SM):
        ...
        for offset_2 in tl.range(0, 512, _BLOCK_SIZE_2, loop_unroll_factor=2, num_stages=1, disallow_acc_multi_buffer=True):
            ...
            for offset_4 in tl.range(0, 512, _BLOCK_SIZE_4, loop_unroll_factor=1, num_stages=3, disallow_acc_multi_buffer=True, flatten=False):
...
def attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sm_scale: float, *, _launcher=_default_launcher):
    ...
    # src[helion_kernels.py:67]:     for tile_m in hl.tile(m_dim):
    # src[helion_kernels.py:65-86]: ...
    _launcher(_helion_attention_bshd, (_NUM_SM,), q, k, v, out, qk_scale, _NUM_SM, _RDIM_SIZE_3, num_warps=16, num_stages=4)
