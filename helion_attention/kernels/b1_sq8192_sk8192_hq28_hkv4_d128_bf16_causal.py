...
def _helion_causal_attention_bshd(q, k, v, out, qk_scale, _NUM_SM: tl.constexpr, _RDIM_SIZE_3: tl.constexpr):
    ...
    out_desc = tl.make_tensor_descriptor(out, [1, 8192, 28, 128], [29360128, 3584, 128, 1], [1, _BLOCK_SIZE_2, 1, 128])
    ...
    if end_pid > total_pids:
        end_pid = total_pids
    for virtual_pid in tl.range(start_pid, end_pid, disallow_acc_multi_buffer=False, flatten=True):
        # src[helion_kernels.py:233]: for b, h in hl.grid([batch, nheads_q]):
        num_blocks_0 = 28
        ...
        for offset_2 in tl.range(0, 8192, _BLOCK_SIZE_2, disallow_acc_multi_buffer=False, flatten=True):
            ...
            for offset_4 in tl.range(0, tl.cast(tile_end, tl.int32), _BLOCK_SIZE_4, disallow_acc_multi_buffer=True, flatten=True):
...
def attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sm_scale: float, *, _launcher=_default_launcher):
    ...
    # src[helion_kernels.py:235]:     for tile_m in hl.tile(m_dim):
    # src[helion_kernels.py:233-270]: ...
    _launcher(_helion_causal_attention_bshd, (_NUM_SM * 32,), q, k, v, out, qk_scale, _NUM_SM, _RDIM_SIZE_3, num_warps=8, num_stages=5)
