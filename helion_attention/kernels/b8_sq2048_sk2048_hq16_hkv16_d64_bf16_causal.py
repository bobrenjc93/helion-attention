...
def _helion_causal_attention_bshd(q, k, v, out, qk_scale, _RDIM_SIZE_3: tl.constexpr):
    ...
    q_desc = tl.make_tensor_descriptor(q, [8, 2048, 16, 64], [2097152, 1024, 64, 1], [1, _BLOCK_SIZE_2, 1, 64])
    ...
    for offset_2 in tl.range(0, 2048, _BLOCK_SIZE_2, disallow_acc_multi_buffer=False):
...
def attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sm_scale: float, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_causal_attention_bshd, (8 * 16,), q, k, v, out, qk_scale, _RDIM_SIZE_3, num_warps=8, num_stages=3)
