...
Regenerate with:
    python tools/generate.py --batch 1 --seqlen 64 --nheads 32 --nheads-kv 8 --head-dim 128 --dtype bf16 --causal

batch=1 seqlen_q=64 seqlen_k=64 nheads=32 (GQA 32:8) head_dim=128 dtype=bf16 causal=True

Autotuning provenance:
    Helion version: 1.4.0
    Config selection: autotuned
    Autotuning wall time: 256.748 s
    Measured time: 0.007488 ms
...
KERNEL_SPEC = {
    'key': 'b1_sq64_sk64_hq32_hkv8_d128_bf16_causal',
    'batch': 1,
    'seqlen_q': 64,
    'seqlen_k': 64,
    'nheads_q': 32,
...
def _helion_causal_attention_bshd(q, k, v, out, qk_scale, _RDIM_SIZE_3: tl.constexpr):
    ...
    for offset_2 in tl.range(0, 64, _BLOCK_SIZE_2, loop_unroll_factor=2, num_stages=1, flatten=True):
        ...
        for offset_4 in tl.range(0, tl.cast(tile_end, tl.int32), _BLOCK_SIZE_4, loop_unroll_factor=4, num_stages=1, disallow_acc_multi_buffer=True):
            ...
            acc_copy_0 = acc_copy
            # src[helion_kernels.py:248]: k_blk = k[b, tile_n, h_kv, :]
            k_blk = tl.load(k + (indices_4[:, None] * 1024 + symnode_0 * 128 + (0 + indices_3)[None, :] * 1), mask_4[:, None], other=0, eviction_policy='evict_last')
            # src[helion_kernels.py:249]: qk = hl.dot(q_blk, k_blk.T, out_dtype=torch.float32) * qk_scale
            ...
            v_blk = tl.load(v + (indices_4[:, None] * 1024 + symnode_0 * 128 + (0 + indices_3)[None, :] * 1), mask_4[:, None], other=0, eviction_policy='evict_last')
...
def attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sm_scale: float, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_causal_attention_bshd, (1 * 32,), q, k, v, out, qk_scale, _RDIM_SIZE_3, num_warps=4, num_stages=5)
