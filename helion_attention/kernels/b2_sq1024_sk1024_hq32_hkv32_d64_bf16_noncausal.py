...
batch=2 seqlen_q=1024 seqlen_k=1024 nheads=32 head_dim=64 dtype=bf16 causal=False

Autotuning provenance:
    Helion version: 1.4.0
    Config selection: autotuned
    Autotuning wall time: 398.819 s
    Measured time: 0.095552 ms
...
KERNEL_SPEC = {
    'key': 'b2_sq1024_sk1024_hq32_hkv32_d64_bf16_noncausal',
    'batch': 2,
    'seqlen_q': 1024,
    'seqlen_k': 1024,
    'nheads_q': 32,
...
def _helion_attention_bshd(q, k, v, out, qk_scale, _NUM_SM: tl.constexpr, _RDIM_SIZE_3: tl.constexpr):
    ...
    for virtual_pid in tl.range(start_pid, end_pid):
        ...
        for offset_2 in tl.range(0, 1024, _BLOCK_SIZE_2, disallow_acc_multi_buffer=True, flatten=True):
            ...
            for offset_4 in tl.range(0, 1024, _BLOCK_SIZE_4, disallow_acc_multi_buffer=False, flatten=True):
                ...
                acc_copy_0 = acc_copy
                # src[helion_kernels.py:190]: k_blk = k[b, tile_n, h_kv, :]
                k_blk = tl.load(k + (offset_0 * 2097152 + indices_4[:, None] * 2048 + offset_1 * 64 + (0 + indices_3)[None, :] * 1), None, eviction_policy='evict_first')
                # src[helion_kernels.py:191]: qk = hl.dot(q_blk, k_blk.T, out_dtype=torch.float32) * qk_scale
                ...
                v_blk = tl.load(v + (offset_0 * 2097152 + indices_4[:, None] * 2048 + offset_1 * 64 + (0 + indices_3)[None, :] * 1), None)
