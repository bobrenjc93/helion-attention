...
Regenerate with:
    python tools/generate.py --batch 8 --seqlen 16 --nheads 8 --head-dim 32 --dtype bf16 --causal

batch=8 seqlen_q=16 seqlen_k=16 nheads=8 head_dim=32 dtype=bf16 causal=True

Autotuning provenance:
    Helion version: 1.4.0
    Config selection: autotuned
    Autotuning wall time: 371.692 s
    Measured time: 0.005920 ms
...
KERNEL_SPEC = {
    'key': 'b8_sq16_sk16_hq8_hkv8_d32_bf16_causal',
    'batch': 8,
    'seqlen_q': 16,
    'seqlen_k': 16,
    'nheads_q': 8,
...
def _helion_causal_attention_bshd(q, k, v, out, qk_scale, _RDIM_SIZE_3: tl.constexpr):
    ...
    end_0 = 16
    for offset_2 in tl.range(0, 16, _BLOCK_SIZE_2, loop_unroll_factor=1, num_stages=3, flatten=True):
        ...
        q_blk = tl.load(q + (offset_0 * 4096 + indices_2[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), mask_2[:, None], other=0, eviction_policy='evict_last')
        # src[helion_kernels.py:244]: key_stop = tile_m.end
        tile_end = tl.minimum(offset_2 + _BLOCK_SIZE_2, end_0)
        ...
        for offset_4 in tl.range(0, tl.cast(tile_end, tl.int32), _BLOCK_SIZE_4, loop_unroll_factor=4, num_stages=4, disallow_acc_multi_buffer=False, flatten=True):
            ...
            acc_copy_0 = acc_copy
            # src[helion_kernels.py:248]: k_blk = k[b, tile_n, h_kv, :]
            k_blk = tl.load(k + (offset_0 * 4096 + indices_4[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), mask_4[:, None], other=0, eviction_policy='evict_last')
            # src[helion_kernels.py:249]: qk = hl.dot(q_blk, k_blk.T, out_dtype=torch.float32) * qk_scale
            ...
            _mask_to = tl.where(mask_2[:, None] & mask_4[None, :], qk_1, tl.full([], float('-inf'), tl.float32))
            ...
            _mask_to_1 = tl.where(mask_2[:, None] & mask_4[None, :], v_8, tl.full([], 0, tl.float32))
            ...
            v_blk = tl.load(v + (offset_0 * 4096 + indices_4[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), mask_4[:, None], other=0, eviction_policy='evict_last')
        ...
        tl.store(out + (offset_0 * 4096 + indices_2[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), v_20, mask_2[:, None])
...
def attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sm_scale: float, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_causal_attention_bshd, (8 * 8,), q, k, v, out, qk_scale, _RDIM_SIZE_3, num_warps=2, num_stages=2)
