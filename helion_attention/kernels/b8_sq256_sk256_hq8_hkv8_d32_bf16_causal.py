...
Regenerate with:
    python tools/generate.py --batch 8 --seqlen 256 --nheads 8 --head-dim 32 --dtype bf16 --causal

batch=8 seqlen_q=256 seqlen_k=256 nheads=8 head_dim=32 dtype=bf16 causal=True

Autotuning provenance:
    Helion version: 1.4.0
    Config selection: autotuned
    Autotuning wall time: 512.752 s
    Measured time: 0.012480 ms
...
KERNEL_SPEC = {
    'key': 'b8_sq256_sk256_hq8_hkv8_d32_bf16_causal',
    'batch': 8,
    'seqlen_q': 256,
    'seqlen_k': 256,
    'nheads_q': 8,
...
def _helion_causal_attention_bshd(q, k, v, out, qk_scale, _RDIM_SIZE_3: tl.constexpr):
    ...
    for offset_2 in tl.range(0, 256, _BLOCK_SIZE_2, num_stages=3, disallow_acc_multi_buffer=False):
        ...
        q_blk = tl.load(q + (offset_0 * 65536 + indices_2[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), None, eviction_policy='evict_last')
        ...
        for offset_4 in tl.range(0, tl.cast(tile_end, tl.int32), _BLOCK_SIZE_4, loop_unroll_factor=2, disallow_acc_multi_buffer=False):
            ...
            acc_copy_0 = acc_copy
            # src[helion_kernels.py:248]: k_blk = k[b, tile_n, h_kv, :]
            k_blk = tl.load(k + (offset_0 * 65536 + indices_4[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), mask_4[:, None], other=0, eviction_policy='evict_last')
            # src[helion_kernels.py:249]: qk = hl.dot(q_blk, k_blk.T, out_dtype=torch.float32) * qk_scale
            ...
            v_blk = tl.load(v + (offset_0 * 65536 + indices_4[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), mask_4[:, None], other=0, eviction_policy='evict_last')
        ...
        tl.store(out + (offset_0 * 65536 + indices_2[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), v_20, None)
