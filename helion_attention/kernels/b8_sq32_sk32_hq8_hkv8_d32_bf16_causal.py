...
Regenerate with:
    python tools/generate.py --batch 8 --seqlen 32 --nheads 8 --head-dim 32 --dtype bf16 --causal

batch=8 seqlen_q=32 seqlen_k=32 nheads=8 head_dim=32 dtype=bf16 causal=True

Autotuning provenance:
    Helion version: 1.4.0
    Config selection: autotuned
    Autotuning wall time: 172.658 s
    Measured time: 0.006016 ms
...
KERNEL_SPEC = {
    'key': 'b8_sq32_sk32_hq8_hkv8_d32_bf16_causal',
    'batch': 8,
    'seqlen_q': 32,
    'seqlen_k': 32,
    'nheads_q': 8,
...
def _helion_causal_attention_bshd(q, k, v, out, qk_scale, _RDIM_SIZE_3: tl.constexpr):
    ...
    for offset_2 in tl.range(0, 32, _BLOCK_SIZE_2, loop_unroll_factor=2, num_stages=1, flatten=False):
        ...
        q_blk = tl.load(q + (offset_0 * 8192 + indices_2[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), None, eviction_policy='evict_first')
        ...
        for offset_4 in tl.range(0, tl.cast(tile_end, tl.int32), _BLOCK_SIZE_4):
            ...
            acc_copy_0 = acc_copy
            # src[helion_kernels.py:248]: k_blk = k[b, tile_n, h_kv, :]
            k_blk = tl.load(k + (offset_0 * 8192 + indices_4[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), mask_4[:, None], other=0, eviction_policy='evict_last')
            # src[helion_kernels.py:249]: qk = hl.dot(q_blk, k_blk.T, out_dtype=torch.float32) * qk_scale
            ...
            v_blk = tl.load(v + (offset_0 * 8192 + indices_4[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), mask_4[:, None], other=0)
        ...
        tl.store(out + (offset_0 * 8192 + indices_2[:, None] * 256 + offset_1 * 32 + (0 + indices_3)[None, :] * 1), v_20, None)
...
def attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sm_scale: float, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_causal_attention_bshd, (8 * 8,), q, k, v, out, qk_scale, _RDIM_SIZE_3, num_warps=4, num_stages=1)
