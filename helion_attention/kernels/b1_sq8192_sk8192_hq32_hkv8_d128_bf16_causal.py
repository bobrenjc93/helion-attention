...
Autotuning provenance:
    ...
    Autotuning wall time: 1492.333 s
    Measured time: 5.442768 ms
...
def _helion_causal_attention_bshd(q, k, v, out, qk_scale, _NUM_SM: tl.constexpr, _RDIM_SIZE_3: tl.constexpr):
    ...
    v_desc = tl.make_tensor_descriptor(v, [1, 8192, 8, 128], [8388608, 1024, 128, 1], [1, _BLOCK_SIZE_4, 1, 128])
    ...
    total_pids = 32 * 1
    for virtual_pid in tl.range(tl.program_id(0), total_pids, _NUM_SM * 16, flatten=False):
        # src[helion_kernels.py:233]: for b, h in hl.grid([batch, nheads_q]):
        num_pid_m = 32
        num_pid_n = 1
        ...
        for offset_2 in tl.range(0, 8192, _BLOCK_SIZE_2, flatten=False):
            ...
            q_blk = tl.load(q + (indices_2[:, None] * 4096 + offset_1 * 128 + (0 + indices_3)[None, :] * 1), None, eviction_policy='evict_last')
